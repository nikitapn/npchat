...
npchat::UserData AuthorizatorImpl::tryLogIn(std::string_view user_email, std::string_view user_password) {
  ...
  if (auto user = userService_->getUserByEmail(user_email); user) {
    if (!user_password.empty() && userService_->checkPassword(*user, user_password) == false) {
      dataObservers_->alarm(npchat::AlarmType::Warning, "User \"" + user->user_name + "\" forgot his password");
      throw npchat::AuthorizationFailed(npchat::AuthorizationFailed_Reason::incorrect_password);
    }
    ...
    dataObservers_->alarm(npchat::AlarmType::Info, "User \"" + user->user_name + "\" has logged in");

    return {user->user_name, sessions_.emplace(std::move(s)).first->sid, oid};
  } else {
    throw npchat::AuthorizationFailed(npchat::AuthorizationFailed_Reason::email_does_not_exist);
  }
}
...
npchat::UserData AuthorizatorImpl::LogIn(::nprpc::flat::Span<char> login, ::nprpc::flat::Span<char> password) {
  ...
  if (user_password.empty()) {
    dataObservers_->alarm(npchat::AlarmType::Warning, "User is trying to log in with an empty password");
    throw npchat::AuthorizationFailed(npchat::AuthorizationFailed_Reason::incorrect_password);
  }
...
npchat::UserData AuthorizatorImpl::LogInWithSessionId(::nprpc::flat::Span<char> session_id) {
  ...
  if (email.empty())
    throw npchat::AuthorizationFailed(npchat::AuthorizationFailed_Reason::session_does_not_exist);
...
void AuthorizatorImpl::AuthorizatorImpl::RegisterStepOne(::nprpc::flat::Span<char> username, ::nprpc::flat::Span<char> email, ::nprpc::flat::Span<char> password) {
  {
    ...
    if (userService_->checkUsername(username) == false)
      throw npchat::RegistrationFailed(npchat::RegistrationFailed_Reason::username_already_exist);
    if (userService_->checkEmail(email) == false)
      throw npchat::RegistrationFailed(npchat::RegistrationFailed_Reason::email_already_registered);
  }
  ...
  dataObservers_->alarm(npchat::AlarmType::Info, "Dear " + user.user->user_name + ", here is your confirmation code: " + std::to_string(user.code));
...
void AuthorizatorImpl::RegisterStepTwo(::nprpc::flat::Span<char> username, uint32_t code) {
  ...
  if (auto it = new_users_db_.find((std::string)username); it != new_users_db_.end()) {
    if (it->second.code != code) {
      throw npchat::RegistrationFailed(npchat::RegistrationFailed_Reason::incorrect_code);
    }
    dataObservers_->alarm(npchat::AlarmType::Info, "New user '" + it->first + "' has been registered");
    ...
    throw npchat::RegistrationFailed(npchat::RegistrationFailed_Reason::invalid_username);
  }
}
