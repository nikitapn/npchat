...
public:
  ...
  std::vector<npchat::Fertilizer> getAll() {
    std::vector<npchat::Fertilizer> fertilizers;
  ...
  std::optional<npchat::Fertilizer> getFertilizer(uint32_t id) {
    std::optional<npchat::Fertilizer> result;
    sqlite3_bind_int(get_stmt_, 1, id);
    if (sqlite3_step(get_stmt_) == SQLITE_ROW) {
      npchat::Fertilizer fertilizer;
